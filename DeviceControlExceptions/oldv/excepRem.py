...
# Function to delete USB device control exceptions
def delete_usb_exceptions(bearer_token, policy_id, combined_ids):
    ...
    payload = {
        "resources": [
            {
                "id": policy_id,
                "name": "CyberSOC - Monitoring test",
                "description": "Updated policy with deleted exceptions",
                "settings": {
                    "delete_exceptions": combined_ids,
                    "classes": [
                        {
                            "id": "MASS_STORAGE",
                            "exceptions": []
...
policy_name = "CyberSOC - Monitoring test"
...
for target_cid in target_cids:
    try:
        ...
        # Delete exceptions for the target CID
        response = delete_usb_exceptions(bearer_token, policy_id, combined_ids)
        print(f"Exceptions deleted for CID {target_cid} under policy '{policy_name}': {response}")
        
        # Verify deletion
        updated_policy = get_policy_details(bearer_token, policy_id)
        print(f"Updated policy details: {json.dumps(updated_policy, indent=2)}")  # Debugging line
    except requests.exceptions.HTTPError as err:
        print(f"HTTP error occurred for CID {target_cid}: {err}")

print("USB device control exceptions deletion process completed.")